...
class FlectraSentryHandler(SentryHandler):
    '''
    Customized :class:`raven.handlers.logging.SentryHandler`.

    Allows to add additional Flectra and HTTP request data to the event
    which is sent to Sentry.
    '''

    def __init__(self, include_extra_context, *args, **kwargs):
        super(FlectraSentryHandler, self).__init__(*args, **kwargs)
    ...
    def emit(self, record):
        ...
        return super(FlectraSentryHandler, self).emit(record)


class SanitizeFlectraCookiesProcessor(SanitizePasswordsProcessor):
    '''
    Custom :class:`raven.processors.Processor`.

    Allows to sanitize sensitive Flectra cookies, namely the
    "session_id" cookie.
    '''
