...
class StockLocation(models.Model):
    ...
    def _compute_loc_accuracy(self):
        for self in self:
            ...
            if history:
                ...
                if history[:wh.counts_for_accuracy_qty]:
                    if len(history) > wh.counts_for_accuracy_qty:
                        self.loc_accuracy = mean(history[
                                                 :wh.counts_for_accuracy_qty].
                                                 mapped('inventory_accuracy'))
                    else:
                        self.loc_accuracy = mean(history.mapped(
                            'inventory_accuracy'))
    ...
    def create_zero_confirmation_cycle_count(self):
        ...
        wh_id = self.get_warehouse()
        date_horizon = wh_id.get_horizon_date().strftime(
        ...
        rule = self.env['stock.cycle.count.rule'].search([
            ('rule_type', '=', 'zero'), ('warehouse_ids', '=', wh_id.id)])
